...
public class ManageSharings implements ManageSharingsRemote {
	...
	public List<Sharing> getNewSharings(Group group) {
		...
		try {
			...
			log.info(result.size() + " sharings returned for group " + group + " (name = " + group.getName() + ", id = "
					+ group.getId() + ")");
		} catch (Exception e) {
			log.error("Getting new sharings of group " + group + " (name = " + group.getName() + ", id = "
					+ group.getId() + ")", e);
	...
	public List<Sharing> getSharingsByUser(User user) {
		...
		try {
			...
			result = query.getResultList();
			log.info(result.size() + " sharings returned for user " + user + " (name = " + user.getUserName()
					+ ", id = " + user.getId() + ")");
		} catch (Exception e) {
			log.error("Getting new sharings of group " + user + " (name = " + user.getUserName() + ", id = "
					+ user.getId() + ")", e);
			result = Collections.emptyList();
		}
	...
	public List<Sharing> getNewSharingsByType(SharingType type, Group group) {
		...
		try {
			...
			result = query.getResultList();
			log.info(result.size() + " sharings returned for group " + group + " (name = " + group.getName() + ", id = "
					+ group.getId() + ") and type " + type.getName());
		} catch (Exception e) {
			log.error("Getting new sharings of group " + group + " (name = " + group.getName() + ", id = "
					+ group.getId() + ") and type " + type.getName(), e);
			result = Collections.emptyList();
		}
	...
	public List<Sharing> getNewSharingsByUserAndType(SharingType type, User user) {
		...
		try {
			...
			result = query.getResultList();
			log.info(result.size() + " sharings returned for user " + user + " (name = " + user.getUserName()
					+ ", id = " + user.getId() + ") and type " + type.getName());
		} catch (Exception e) {
			log.error("Getting new sharings of group " + user + " (name = " + user.getUserName() + ", id = "
					+ user.getId() + ") and type " + type.getName(), e);
			result = Collections.emptyList();
		}
	...
	public int setInvalid(Sharing sharing) {
		...
		try {
			...
			log.info("State od sharing change to not active " + sharing + " (id = " + sharing.getId() + ", user "
					+ sharing.getUser().getId() + ")");
			return query.executeUpdate();
		} catch (Exception e) {
			log.error("setting state of sharing " + sharing + " (id = " + sharing.getId() + ", user "
					+ sharing.getUser().getId() + ")", e);
		}
